package com.oseasy.cas.modules.cas.security;
/*
 ...
 * @since 1.2
 */
@Service("frontCasLogoutFilter")
public class FrontCasLogoutFilter extends AdviceFilter {

    private static final Logger log = LoggerFactory.getLogger(FrontCasLogoutFilter.class);
    ...
    protected boolean preHandle(ServletRequest request, ServletResponse response) throws Exception {
        ...
        try {
//            CoreUtils.removeAll();
//            TenantConfig.clearCache(request, response);
//            subject.logout();

            CoreUtils.logout(subject, request, response);
    ...
    protected String getRedirectUrl(ServletRequest request, ServletResponse response, Subject subject) {
        String rlogoutUrl = CasSval.getCasLogoutUrl();
        if(CasSval.getCasLoginOpen()){
            this.redirectUrl = (StringUtil.isEmpty(rlogoutUrl) ? getRedirectUrl() : rlogoutUrl);
        }
