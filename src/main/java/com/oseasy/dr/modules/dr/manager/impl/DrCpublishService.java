 ...
 */
@Service
public class DrCpublishService implements IManager, IManagerExt {
    ...
    public Integer getOperType() {
        return OperType.PUBLISH.getKey();
    }
    ...
    public void call(DrCardCparam param) {
        logger.error(">>>设备["+param.getDatas().getEtpId()+"]卡["+param.getDatas().getCardId()+"]，响应了一次-成功");
        ...
        if(!(getOperType()).equals(rparam.getOperType())){
            logger.error(">>>设备["+param.getDatas().getEtpId()+"]卡["+rparam.getCardId()+"]，操作类型不匹配-成功->"+rparam.getSuccess());
            return;
        }
        ...
        if(isFinish){
            ...
            System.out.println(StringUtil.mapKeyInList(cvo2.getEtpCerspaces().keySet(), cvo2.getEtpIds()));
            ...
            System.out.println("==============================================================Clear is null ? cvo="+ cvo3);
        }else{
        ...
        logger.warn(JsonMapper.toJsonString(card));
        logger.warn("*******************************************************************************************************");
    }
    ...
    public void callFail(DrCardCparam param) {
        logger.error(">>>设备["+param.getDatas().getEtpId()+"]卡["+param.getDatas().getCardId()+"]，响应了一次-失败");
        //TODO 需要考虑的问题，会不会存在设备回调比系统处理业务快（可能导致回调取不到卡）
        //1、获取响应信息,包含卡信息和执行状态
        DrCardRparam rparam = param.getDatas();
         ...
         */
        if(!(getOperType()).equals(rparam.getOperType())){
            logger.error(">>>设备["+param.getDatas().getEtpId()+"]卡["+rparam.getCardId()+"]，操作类型不匹配-失败->"+rparam.getSuccess());
            return;
        }
        ...
        if(cvo == null){
           logger.error(">>>设备["+param.getDatas().getEtpId()+"]，处理完毕-失败->"+rparam.getSuccess());
        	return;
        }
        Boolean isFinish = cvo.getHasFinish();
        if(isFinish){
        ...
        logger.warn("***************************************设备号："+rparam.getEtpSn() + "处理完成***************************************");
        logger.warn(JsonMapper.toJsonString(card));
        logger.warn(JsonMapper.toJsonString(param));
        logger.warn("*******************************************************************************************************");
    }
    ...
    public ApiTstatus<DrCardRparam> runner(DrCardParam param) {
        ...
        try {
            ...
            logger.error(">>>设备["+param.getEtpId()+"]，请求了一次");
            ...
            return new ApiTstatus<DrCardRparam>(true, "设备无法连接或命令对象初始化异常：设备IP、NO、PSW、TYPE不能为空(或设备号、密码不符合规范)！", rparam);
        }
