 ...
 */
public class Role extends UserEntity<Role> implements IidEntity, IRole {
	...
	private Boolean rinit; //是否为初始角色:0、默认（否）；1、是
	...
	private String auth;// 权限数值 值越小权限越大  RoleAuthEnum
...
//	private List<User> userList = Lists.newArrayList(); // 拥有用户列表
	...
	public Role() {
		...
		this.useable=Const.YES;
		this.rinit = false;
	}

	public Role(Office office) {
		super();
		this.office = office;
		this.rinit = false;
	}
	...
	public Boolean getRinit() {
		return rinit;
	}

	public void setRinit(Boolean rinit) {
		this.rinit = rinit;
	}
	...
	public String getAuth() {
		if((this.auth == null) && StringUtils.isNotEmpty(this.rtype)){
			this.auth = this.rtype;
			if(StringUtils.isNotEmpty(this.auth)){
				this.auth = CoreSval.Rtype.OTHER.getKey();
			}
		}
		return auth;
	}
