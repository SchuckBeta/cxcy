 ...
 *
 */
public class FindDictByTenantUtil {
    /**
     * 检查类指定方法是否存在@FindDictByTenant注解.
     * @param mstr 方法签名
     */
        ...
        try {
            ...
            for(String classNames : classNameList){
                ...
                for (Method method : methods) {
                    if((method == null) || (!(method.getName().equals(mstr)))){
                        continue;
                    }
                    // 如果此方法有注解，就把注解里面的数据赋值到user对象
                    if (method.isAnnotationPresent(FindDictByTenant.class) && (method.getAnnotation(FindDictByTenant.class) != null)) {
                        return true;
                    }
                }
            }
