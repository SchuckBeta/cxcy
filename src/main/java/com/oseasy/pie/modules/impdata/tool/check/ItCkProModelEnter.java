 ...
 * @author chenhao
 */
public class ItCkProModelEnter implements IitCheck<ItCparamPm>{
    public final static Logger logger = Logger.getLogger(ItCkProModelEnter.class);
    private String format;

    public ItCkProModelEnter(String format) {
        super();
        this.format = format;
    }
        ...
        if((pe instanceof ProModelError) && (pev instanceof ProModelError)){
            ...
            if(StringUtil.isNotEmpty(param.getVal())){
                try {
                    ...
                    phe.setEnter(param.getVal());
                    validinfo.setEnter(param.getVal());
