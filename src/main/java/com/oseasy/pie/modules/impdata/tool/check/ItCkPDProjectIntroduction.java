 ...
 *
 */
public class ItCkPDProjectIntroduction implements IitCheck<ItCparamPm>{
    public static Logger logger = Logger.getLogger(ItCkPDProjectIntroduction.class);
    ...
    public ItCparamPm validate(ItCparamPm param, IitCheckEetyExt pe, IitCheckEetyExt pev) {
        ...
        if (StringUtil.isEmpty(param.getVal())) {
            ...
            iie.setErrmsg("必填信息");
        }else if ((param.getVal()).length() > 5000) {
            param.setTag(param.getTag() + 1);
