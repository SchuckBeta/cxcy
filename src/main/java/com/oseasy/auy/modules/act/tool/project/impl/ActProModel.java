...
//			Menu  menu= coreService.getMenu(AuyIds.SITE_MENU_PROJECT_ROOT.getId());
			if (menu!=null && actYw.getGroupId() != null) {
...
//							    isFront = ActYwGform.checkFront(isFront, actYwGnodeService.findListBygYwGprocess(actYw.getGroupId(),actYwGnodeIndex.getId()));
							...
							menuNextForm.setLtype(TenantMenu.S3.getId());
...
//												getRoleByRid(SysIds.EXPERT_ROLE.getId());
										if (role != null) {
				...
				menuQueryForm.setLtype(TenantMenu.S3.getId());
...
//				}
   ...
   @Override
	...
	public Boolean dealCategory(ActProParamVo actProParamVo) {
		...
		ActYw actYw = actProParamVo.getActYw();
		//修改栏目为可见
		//Category category = categoryService.get(CmsIds.SITE_CATEGORYS_PROJECT_ROOT.getId());
...
//		Category category =categoryService.getByIdAndType(site.getId(), CategoryType.CT_PRO_APP.getKey());
		if(category!=null){
			//默认添加申报表单
	...
	//		categoryapp.setInList(CoreSval.SHOW);
			...
			categoryapp.setLtype(TenantCategory.S3.getId());
