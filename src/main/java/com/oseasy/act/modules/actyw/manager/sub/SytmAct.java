 ...
 */
public class SytmAct extends SupSytm<SytmvAct> {
    public final static Logger logger = Logger.getLogger(SytmAct.class);
    ...
    public SytmAct(SytmvAct sytmvo) {
        super(sytmvo);
    }
    ...
    public boolean run() {
        ...
        pactYwGroup.setTenantId(sysTenant.getId());
...
//        actYwGroupDao.update(this.sytmvo.getGroup());
         ...
         */
        Role prole = new Role();
        prole.setTenantId(sytmvTenant.getTenantId());
        List<Role> roles = roleDao.findRoles(prole);
        /**
         * 查找当前租户角色.
         */
        Role ptrole = new Role();
        ptrole.setTenantId(sysTenant.getId());
        List<Role> troles = roleDao.findRoles(ptrole);
        ...
        for (ActYwGrole curg: actYwGroles) {
            ...
            for (Role curRole: roles) {
                logger.info("curg.getRole().getId() = "+ curg.getRole().getId());
                logger.info("curRole.getRid() = "+ curRole.getRid());
                logger.info("curg.getRole().getId() =? curRole.getRid() = "+ ((curg.getRole().getId()).equals(curRole.getRid())));
                if((curg.getRole().getId()).equals(curRole.getRid()) || (curg.getRole().getId()).equals(curRole.getId())){
                    curg.setRole(curRole);
                    cflag = false;
                    break;
                }
            }
            ...
            curg.setCreateBy(new User(CoreIds.NCE_SYS_USER_SUPER.getId()));
            curg.setCreateDate(DateUtil.newDate());
            curg  = updateGrole(this.sytmvo.getGroup(), curg, curGnodevs, troles);
...
//        }
    ...
    public static ActYwGrole updateGrole(ActYwGroup group, ActYwGrole g, List<ActYwGnodev> mvs, List<Role> troles) {
        for (ActYwGnodev mv : mvs) {
            if((g.getGnode() != null) && ((mv.getId()).equals(g.getGnode().getId()))){
                g.getGnode().setId(mv.getNid());
            }
         ...
         * 处理固定的角色ID.
         */
        if(StringUtil.checkNotEmpty(troles)){
            for(Role tr :troles){
                if(StringUtil.isEmpty(tr.getId()) || StringUtil.isEmpty(tr.getRid())){
                    continue;
                }

                if((tr.getId()).equals(g.getRole().getRid())){
                    group.setUiHtml(group.getUiHtml().replaceAll(StringUtil.SHUANGYH + tr.getRid() + StringUtil.SHUANGYH, StringUtil.SHUANGYH + g.getRole().getId() + StringUtil.SHUANGYH));
                    group.setUiJson(group.getUiJson().replaceAll(StringUtil.SHUANGYH + tr.getRid() + StringUtil.SHUANGYH, StringUtil.SHUANGYH + g.getRole().getId() + StringUtil.SHUANGYH));
                    break;
                }
            }
        }
     ...
     * @return
     */
    public static List<String> getRoleNameByGnodeId(String gnodeId){
        if (StringUtils.isBlank(gnodeId)){
            return new ArrayList<>();
        }
        return roleDao.getRoleNameByGnodeId(gnodeId);
    }
}
