...
<head>
    ...
    <script src="${ctxJs}/frontCyjd/uploaderFile.js"></script>
</head>
        ...
        <div id="teamInfo" class="panel-body collapse in">
            <div class="panel-inner">
                <div class="row-fluid row-info-fluid">
			 	...
			 	<c:if test="${fn:length(proModelMdGc.gmembers) <= 0}">
                ...
                <table class="table table-bordered table-condensed table-orange table-center table-nowrap table-hover">
                    ...
                    <tbody id="teamTableStudent">
                    	<c:forEach items="${proModelMdGc.gmembers}" var="member" varStatus="status">
	                        <tr>
	                            <td>${status.index +1}</td>
	                            <td>${member.name}</td>
	                            <td>${member.no}</td>
                ...
                <c:if test="${fn:length(proModelMdGc.gteachers) <= 0}">
                ...
                <table
                    ...
                    <tbody id="teamTableTeacher">
                    <c:forEach items="${proModelMdGc.gteachers}" var="teacher" varStatus="status">
                        <tr>
                            <td>${status.index +1}</td>
                            <td>${teacher.name}</td>
                            <td>${teacher.no}</td>
				...
				<c:if test="${fn:length(proModelMdGc.gbusinfos) > 0}">
                <div class="table-caption">工商信息</div>
                <table
                        class="table table-bordered table-condensed table-hover table-center table-orange table-nowrap">
                    <thead>
                    <tr>
                    ...
                    <tbody id="teamTableTeacher">
                    <c:forEach items="${proModelMdGc.gbusinfos}" var="gbusinfo" varStatus="status">
                        <tr>
                            <td>${status.index +1}</td>
                            <td>${gbusinfo.cyname}</td>
                            <td>${gbusinfo.area}</td>
                            <td>${gbusinfo.no}</td>
                            <td>${gbusinfo.name}</td>
                            <td>${gbusinfo.money}</td>
                            <td>${gbusinfo.regTime}</td>
                        </tr>
                    </c:forEach>
                    </tbody>
                </table>
...
<c:if test="${fn:length(sysAttachments) > 0}">
