...
<div id="app" v-show="pageLoad" class="container-fluid mgb-60" style="display: none">
        ...
        <el-form :model="creditAuditForm" ref="creditAuditForm" :rules="creditAuditRules" :disabled="disabled"
            ...
            <el-form-item v-if="!isSkill" label="相同项目：" prop="appIds">
                <el-select v-model="creditAuditForm.appIds" filterable placeholder="请选择" @change="handleChangeAppId">
                    <el-option v-if="scoRdetail.maxOrSum === '2' || !hasSpro" label="无" value="-1"></el-option>
                    <el-option v-for="item in myPros" :label="item.name" :value="item.id" :key="item.id"></el-option>
                </el-select>
                <span v-if="!isAtypeDisabled" class="el-form-item-expository">同一个项目记最高分值</span>
            </el-form-item>
            ...
            <el-form-item label="审核：" prop="atype" style="margin-bottom: 0">
                <el-col :span="10">
                    <el-form-item label-width="0" prop="atype">
                        <el-tooltip content="已申请过相同的认定学分" :disabled="!isAtypeDisabled" popper-class="white"
                                    placement="bottom">
                            <el-select :disabled="isAtypeDisabled" v-model="creditAuditForm.atype">
    ...
    <el-button type="primary" @click.stop.prevent="validatePwEnterAuditForm" :disabled="disabled"
            ...
            <div v-if="creditApply.rdetail && creditApply.rdetail.remarks" class="credit-audit-tip">
                <p>认定标准及备注：</p>
...
<script type="text/javascript">
    ...
    new Vue({
        ...
        computed: {
            isAtypeDisabled: function () {
                return this.scoRdetail.maxOrSum === '2' && this.creditAuditForm.appIds != '' && this.creditAuditForm.appIds != '-1'
            },
        ...
        methods: {
            ...
            handleChangeAppId: function (appId) {
                ...
                if (appId == '-1') {
                    ...
                    this.creditAuditForm.atype = '';
                    return
                }
                if (this.scoRdetail.maxOrSum === '2') {
                    this.creditAuditForm.atype = '0'
                }
...
//                        self.validateScoreOther();
            ...
            validateScoreOther: function () {
                ...
                if (this.creditAuditForm.atype === '1') {
                    ...
                    this.disabled = true;
                    this.$axios.get('/scr/scoRapply/ajaxScoSum?id=' + this.scrId+'&rdetail.id='+rdetail.id).then(function (response) {
                        var data = response.data;
                        var entity = {};
                        if (data.status === 1) {
                            data = data.data || [];
                            ...
                            if (rdetail.isJoin === '1') {
                                var rDetailTotal = parseFloat(creditAuditForm.score);
                                if (rDetailVal === 1) {
                                    hasRDetailError = true;
                                    self.generateErrorText({user: user}, 'rDetailVal')
                                }
                            }
                            if (globalCalculateRules.isSnumlimit === '1') {
                                var sumValTotal = parseFloat(creditAuditForm.score);
                                if (sumValTotal > parseFloat(globalCalculateRules.snumlimit)) {
                                    hasGlobalCRError = true;
                                    self.generateErrorText({user: user}, 'snumlimit')
                                }
                            }
                        ...
                        this.$axios.get('/scr/scoRapply/ajaxScoSum?id=' + this.scrId+'&rdetail.id='+rdetail.id).then(function (response) {
                            ...
                            if (data.status === 1) {
                                ...
                                while (i < memberListLowScoMax.length) {
                                    ...
                                    if (rdetail.isJoin === '1') {
//                                        var rDetailTotal = rDetailVal;
                                        if (rDetailVal === 1) {
                                            hasRDetailError = true;
                                            self.generateErrorText(memberItem, 'rDetailVal')
                                        }
                                    }
                                    if (globalCalculateRules.isSnumlimit === '1') {
                                        var sumValTotal = sumVal;
                                        if (sumValTotal > parseFloat(globalCalculateRules.snumlimit)) {
                                            hasGlobalCRError = true;
                                            self.generateErrorText(memberItem, 'snumlimit')
                                        }
                                    }
                                    i++;
                                }
...
//                                    console.log(self.creditAuditForm.remarks)
            ...
            generateErrorText: function (memberItem, type) {
                if (type === 'rDetailVal') {
//                        this.totalMessageError.push('<p>' + memberItem.user.name + '累计已获得标准分' + score + '，累计此次分值将超过最大标准分<span class="red">' + upVal + '</span>分</p>')
                    this.totalMessageError.push('<p>' + memberItem.user.name + '累计已获得标准学分已满</p>')
                } else {
                    this.totalMessageError.push('<p>' + memberItem.user.name + '累计已获得认定学分已满</p>')
//                        this.totalMessageError.push('<p>' + memberItem.user.name + '累计已获得认定学分已满' + score + '，累计此次认定分值将超过最大认定总分<span class="red">' + upVal + '</span>分</p>')
                }
            },
            ...
            getParams: function () {
                ...
                if (this.isSkill) {
                    return {
                        ...
                        ignodeId: creditAuditForm.ignodeId,
            ...
            submitCreditAuditForm: function () {
                var self = this;
                this.disabled = true;
                this.$axios.post('${iaurl.asave}', this.getParams()).then(function (response) {
                    var data = response.data;
                    if (data.status === 1) {
                        ...
                        self.$msgbox({
                            ...
                            location.href = '/a${ilurl}';
                        }).catch(function () {
                        });
                        <%--window.parent.sideNavModule.changeUnreadTag('${iactyw.id}');--%>
                    } else {
            ...
            getMyPros: function () {
                ...
                this.$axios.get('/scr/scoRapply/ajaxProjects?id=' + this.scrId).then(function (response) {
                    var data = response.data;
                    if (data.status === 1) {
                        self.myPros = data.data.datas || [];
                        self.hasSpro = data.data.isTrue;
                    }
                })
            },
