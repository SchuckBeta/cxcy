...
<div id="app" v-show="pageLoad" style="display: none" class="container-fluid mgb-60">
    ...
    <el-form :model="searchListForm" size="mini" autocomplete="off" ref="searchListForm">
        ...
        <div class="search-block_bar clearfix">
            <div class="search-btns search-btns-font16">
                ...
                <button-export :spilt-prefs="spiltPrefs"
                               :spilt-posts="spiltPosts" label="项目"
                               :search-list-form="searchListForm"></button-export>
                <button-export-file :menu-name="menuName" label="项目"
                ...
                <%--<shiro:hasPermission name="cert:sysCert:make">--%>
                    <%--<el-button size="mini" type="primary" @click.stop="toCertMakePage">证书下发</el-button>--%>
                <%--</shiro:hasPermission>--%>
            </div>
    ...
    <div class="table-container">
        <el-table size="mini" :data="projectListFlatten" class="table" ref="multipleTable" v-loading="loading"
            ...
            <el-table-column label="获奖情况" align="center" prop="type" min-width="110" sortable="final_result">
                <template slot-scope="scope">
                    <span></span>
                    <div v-for="item in scope.row.proModelMdGcHistoryList">{{item.type |
                        selectedFilter(dSTypesEntries)}}：
                        <award-result :type="item.type" :result="item.result"></award-result>
                    </div>
                    <%--{{scope.row.finalResult}}--%>
                </template>
            </el-table-column>
                ...
                <template slot-scope="scope">
                    <shiro:hasPermission name="promodel:promodel:modify">
                        <div class="table-btns-action">
                            <el-button v-if="scope.row.impdata !== '1'" type="text" size="mini" @click.stop.prevent="goToProcessTrack(scope.row)">进度跟踪
                            </el-button>
...
//                    'proModel.endDate': '',
        ...
        computed: {
            ...
            dSTypesEntries: function () {
                return this.getEntries(this.dSTypes)
            },
            ...
            projectListFlatten: {
                get: function () {
                    ...
                    for (var i = 0; i < projectList.length; i++) {
                        ...
                        proModel.proModelMdGcHistoryList = projectList[i].proModelMdGcHistoryList || [];
        ...
        methods: {
            toCertMakePage: function () {
                location.href = "/a/certMake/list?actywId=" + this.searchListForm.actywId;
            },
                ...
                this.$confirm('此操作将删除这条记录, 是否继续?', '提示', {
                    ...
                    self.$axios({
                        ...
                        if (data.ret == '1') {
                            ...
                            window.parent.sideNavModule.changeStaticUnreadTag("/a/promodel/proModel/getTaskAssignCountToDo?actYwId=" + self.searchListForm.actywId);
            ...
            getDSTypes: function () {
                var self = this;
                this.$axios.get('/sys/dict/getDictList?type=0000000169').then(function (response) {
                    var data = response.data;
                    self.dSTypes = data || [];
                })
            }
