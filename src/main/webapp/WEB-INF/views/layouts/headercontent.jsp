...
<style>
    ...
    .sys-top-header.header-bg-yunying{
        background: #e9432d url('/images/managetoplogo_yunying.jpg') no-repeat left top;
    }
    .sys-top-header.header-bg{
        background: #e9432d url('/images/managetoplogo_01.png') no-repeat left top;
    }
    .sys-top-header .el-dropdown-link{
        cursor: pointer;
        color: white;
    }
    .el-dropdown-menu {
...
<!--------顶部------->
<div id="tenantApp" class="sys-top-header" :class="{'header-bg': currTconfig.currpn != 1,'header-bg-yunying': currTconfig.currpn == 1}">
    <ul>
        <li class="remove-nav-index logout"><a href="${ctx}/logout"><i class="iconfont icon-tuichu"></i> 退出</a></li>
        <li class="remove-nav-index"><a id="fs" href="${ctx}/sysMenuIndex"><i class="iconfont icon-ai207"></i> 返回首页 </a></li>
        <li class="remove-nav-index"><a href="${ctx}/sys/menu/treePlusByLtype?lver=2&ltype=55&href=/sys/user/modifyPwd"><i
                class="iconfont icon-user"></i> ${fns:getUser().name}</a></li>
        <li class="remove-nav-index"><a href="${ctx}/sys/menu/treePlusByLtype?lver=2&ltype=55&href=/oa/oaNotify/msgRecList"><i
                class="iconfont icon-iconset0218"></i>消息(<i style="font-style: normal"
                                            id="84bcd363e9b24e25b8aaad7bed3d1dcf_unread">0</i>)</a></li>
        <li class="remove-nav-index" v-if="isShowTenant && !schoolLoading && allSchools.length > 0">
            <%--<el-select v-model="tenantId" placeholder="请选择" size="mini" @change="changeTenant" ref="tenantSelect">--%>
                <%--<el-option v-for="(item, i) in allSchools" :key="i" :label="item.schoolName" :value="item.tenantId"></el-option>--%>
            <%--</el-select>--%>

            <el-dropdown size="mini" @command="changeTenant" placement="bottom">
                <span class="el-dropdown-link">
                    {{currentSchoolName}}<i class="el-icon-arrow-down el-icon--right"></i>
                </span>
                <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item v-for="(item, i) in allSchools" :key="i" :command="item.tenantId"
                                      :class="{red: currentSchoolName == item.schoolName}">{{item.schoolName}}</el-dropdown-item>
                </el-dropdown-menu>
            </el-dropdown>
...
<script type="text/javascript">
    ...
    var tenantApp = new Vue({
        ...
        computed: {
            allSchoolsObj: function() {
                var obj = {}, arr = this.allSchools;
                arr.forEach(function (item) {
                    obj[item.tenantId] = item;
                });
                return obj;
            }
        },
        methods:{
            changeTenant:function (command) {
                var self = this, id = this.allSchoolsObj[command].id;
                this.$axios.get('/sys/tenant/ajaxChange?id=' + id).then(function (response) {
            ...
            getData:function () {
                ...
                this.$axios.get('/sys/tenant/schoolList').then(function (response) {
                    ...
                    self.currentSchoolName = self.currTenantId && self.allSchoolsObj[self.currTenantId] ? self.allSchoolsObj[self.currTenantId].schoolName : '请选择租户';
                    self.schoolLoading = false;
                    if(!tenantDialog){
                        return false;
                    }
                    data.forEach(function (item) {
                        if(item.id == self.nprId){
                            tenantDialog.provinceId = item.id;
                            tenantDialog.provinceName = item.schoolName;
                        }
                    });
                    data = data.filter(function (item) {
                        return item.id != self.nprId;
                    });
                    tenantDialog.allSchools = data || [];
                }).catch(function (error) {
                    self.schoolLoading = false;
                    self.$message({
                        message: self.xhrErrorMsg,
